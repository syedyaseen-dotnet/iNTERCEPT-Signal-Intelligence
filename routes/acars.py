...
def check_acars_tools() -> Response:
    ...
    return jsonify({"acarsdec": has_acarsdec, "ready": has_acarsdec})
...
def start_acars() -> Response:
    ...
    try:
        ...
        thread = threading.Thread(target=stream_acars_output, args=(process, is_text_mode), daemon=True)
        thread.start()

        return jsonify({"status": "started", "frequencies": frequencies, "device": device, "gain": gain})
